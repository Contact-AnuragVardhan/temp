function JSDataGrid(parentElementId,id,width,height,columns,title,enableMouseHover,enableMultiSelection)
{
	     ...
	     if(dataSet)
	     {
	    	 for (var rowIndex = 0; rowIndex < dataSet.length; rowIndex++)
		     {
		        ...
		        if(item.hasOwnProperty(this.childField) && item[this.childField]  && item[this.childField].length > 0)
	            {
		        	...
		        	{
		        		var compArrow = this.getExpandCollapseComponent(row);
		        		this.hideShowRow(totalRowCount,compArrow,false);
	...
	this.getExpandCollapseComponent = function(row)
	{
		var compArrow  = row;
		while(!hasStyleClass(compArrow,"arrow-down") && !hasStyleClass(compArrow,"arrow-right"))
		{
			compArrow = getFirstChild(compArrow);
		}
		if(compArrow && (hasStyleClass(compArrow,"arrow-down") || hasStyleClass(compArrow,"arrow-right")))
		{
			return compArrow;
		}
		return null;
